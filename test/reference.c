...
static inline uint32_t waterhash(const void* key, uint32_t len, uint64_t seed){
    ...
    for (i = 0; i + 16 <= len; i += 16, p += 16) {
        seed = _watermum(
			_watermum(_waterr32(p) ^ _waterp1,
                   _waterr32(p + 4) ^ _waterp2) + seed,
			_watermum(_waterr32(p + 8) ^ _waterp3,
                   _waterr32(p + 12) ^ _waterp4));
    }
	seed += _waterp5;
	switch (len & 15) {
...
uint32_t c_waterhash (const void* key, uint32_t len, uint64_t seed)
{
